...
export default function Templates() {
  const [templates, setTemplates] = useState<null | Tables<"tour_templates_overview">[]>(
    null
  );
        ...
        <TableBody>
          {templates.map((template) => (
            <TableRow key={template.id}>
              ...
              <TableCell>
                {template.newest_version ? (
                  <Badge variant="default">v{template.newest_version}</Badge>
                ) : (
                  <em>No version released yet</em>
                )}
              </TableCell>
              <TableCell>
                {template.latest_tag ? (
                  <Badge variant={template.latest_tag}>{template.latest_tag}</Badge>
                ) : (
                  <em>No version created yet</em>
                )}
              </TableCell>
                ...
                {(user as FullUser).staff_role === "admin" && (
                  <>
                    <Button
                      ...
                      onClick={() => handleDelete(template.id as number)}
                    >
                      Löschen
                    </Button>
                  </>

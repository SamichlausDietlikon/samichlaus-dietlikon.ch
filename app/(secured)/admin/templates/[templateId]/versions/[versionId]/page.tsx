  ...
  useEffect(() => {
    ...
    async function getVersionData() {
      const { data, error } = await supabase
        ...
        .select()
        .eq("id", parseInt(Array.isArray(versionId) ? versionId[0] : versionId))
        .maybeSingle();
      if (error) {
        toast.error(`Fehler: ${JSON.stringify(error)}`);
        return;
  ...
  return loading || !template || !version ? (
    ...
    <section className="max-w-7xl m-auto px-8">
      ...
      <h1 className="text-2xl my-8">
        Version <strong>v{version.version}</strong> der Vorlage{" "}
        <strong>{template.title}</strong> bearbeiten
      </h1>

...
export default function Members() {
  const [members, setMembers] = useState<[]|(Tables<"season_members"> & { users: Tables<"users">[] })[]>([])
  ...
  useEffect(() => {
    supabase.from("season_members").select("*, users!innner(*)").then(({ data, error }) => {
      if (error) {
        console.error(error)
        return
      }
        ...
        <TableBody>
          {members?.map((member) => (
            <TableRow key={member.id}>
              <TableCell>{member.users[0].first_name ?? "-"}</TableCell>
              <TableCell>{member.users[0].last_name ?? "-"}</TableCell>
              <TableCell>{seasonStaffRoles[member.staff_role]}</TableCell>
              <TableCell className="text-right">
                {/* {(user as FullUser).staff_role === "admin" && member.users[0].id !== user?.id && (
                  <UsersFormDialog user={member} refetch={refetch} setRefetch={setRefetch} />
                )} */}
              </TableCell>
            </TableRow>
          ))}

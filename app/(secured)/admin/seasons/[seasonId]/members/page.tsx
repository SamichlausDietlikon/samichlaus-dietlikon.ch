...
export default function Members({params}: {params: {seasonId: string}}) {
  const [members, setMembers] = useState<[]|(Tables<"season_members"> & { users: Tables<"users">|null })[]>([])
  ...
  useEffect(() => {
    supabase.from("season_members").select("*, users!inner(*)").eq("season_id", params.seasonId).then(({ data, error }) => {
      if (error) {
        console.error(error)
        return
      }
          ...
          {members?.map((member) => (
            <TableRow key={member.id}>
              ...
              <TableCell className="text-right">
                {(user as FullUser).staff_role === "admin" && member.users?.id !== user?.id && (

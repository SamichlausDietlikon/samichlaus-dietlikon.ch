...
export default function VillageFormDialog({ village }: { village?: Tables<"villages"> }) {
  ...
  async function handleCreate() {
   const {status, error} = await supabase.from("villages").insert({ name: villageName })
  ...
  async function handleSave() {
    const {status, error} = await supabase.from("villages").update({ name: villageName }).eq("id", village!.id)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          {village
            ? <DialogTitle>Bearbeite <em>{village.name}</em></DialogTitle>
            : <DialogTitle>Erstelle ein Dorf</DialogTitle>
          }
        </DialogHeader>
        <div>
          <Label>Name</Label>
          <Input type="text" value={villageName} onChange={e => setVillageName(e.target.value)} placeholder="Dietlikon" />
        </div>
        <DialogFooter>

...
export default function ResponsibilityFormDialog({
  ...
  responsibility?: Tables<"responsibilities">;
  refetch: boolean;
  ...
  const [responsibilityTimeOverlapping, setResponsibilityTimeOverlapping] = useState(
    responsibility?.time_overlapping || false
  );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          {responsibility ? (
            <DialogTitle>
              Bearbeite <em>{responsibility.name}</em>
            </DialogTitle>
          ) : (
        ...
        <div className="flex items-center gap-2">
          <Input
            type="checkbox"
            checked={responsibilityTimeOverlapping}
            onChange={(e) =>
              setResponsibilityTimeOverlapping(!responsibilityTimeOverlapping)
            }
            placeholder="Chlaus"

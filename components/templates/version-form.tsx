...
export default function VersionForm({ template, tourVersion }: {template: Tables<"tour_templates">, tourVersion?: Tables<"tour_template_versions">}) {
  ...
  async function handleSave() {
    ...
    const {status, error} = await supabase.from("tour_template_versions").update({
      tag,
      version,
      description,
      template: JSON.parse(versionCode)
          ...
          <div className="flex flex-col space-y-1.5">
            ...
            <Select onValueChange={tag => setTag(tag as Enums<"tour_template_tags">)} defaultValue={tag}>
              ...
              <SelectContent>
                <SelectGroup>
                  {
                    Object.entries(tourTemplateTags).slice(
                      tourVersion ? Object.keys(tourTemplateTags).findIndex(t => t === tourVersion!.tag) : 0,
      ...
      {(tourVersion?.tag === "development" || !tourVersion) && (
        <>
          ...
          <div className="space-y-1.5">
            <Label>Vorlage</Label>
            <Editor height="48rem" theme="vs-dark" width="100%" defaultLanguage="json" value={versionCode} onChange={code => setVersionCode(code)} />
          </div>
        </>
      )}
    </div>

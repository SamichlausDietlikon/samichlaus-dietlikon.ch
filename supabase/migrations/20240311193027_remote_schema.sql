...
alter table "public"."user_staff_roles" drop constraint "public_user_staff_roles_user_id_fkey";
...
alter table "public"."user_staff_roles" drop column "user_id";

alter table "public"."user_staff_roles" add column "id" uuid not null;

CREATE UNIQUE INDEX user_staff_roles_pkey ON public.user_staff_roles USING btree (id);

alter table "public"."user_staff_roles" add constraint "user_staff_roles_pkey" PRIMARY KEY using index "user_staff_roles_pkey";

alter table "public"."user_staff_roles" add constraint "user_staff_roles_id_fkey" FOREIGN KEY (id) REFERENCES auth.users(id) ON DELETE CASCADE not valid;

alter table "public"."user_staff_roles" validate constraint "user_staff_roles_id_fkey";
...
using ((auth.uid() IN ( SELECT user_staff_roles.id AS user_id
   FROM user_staff_roles
  WHERE ((user_staff_roles.staff_role = 'admin'::staff_roles) OR (user_staff_roles.staff_role = 'tour_manager'::staff_roles) OR (user_staff_roles.id = auth.uid())))));
...
for insert
to public
with check (((auth.uid() = id) AND (staff_role = 'admin'::staff_roles)));
...
for select
to public
using (((auth.uid() = id) AND (staff_role = 'admin'::staff_roles)));
...
for update
to public
using (((auth.uid() = id) AND (staff_role = 'admin'::staff_roles) AND (id <> auth.uid())))
with check (((auth.uid() = id) AND (staff_role = 'admin'::staff_roles) AND (id <> auth.uid())));
...
using ((auth.uid() IN ( SELECT user_staff_roles.id AS user_id
   FROM user_staff_roles
  WHERE ((user_staff_roles.staff_role = 'admin'::staff_roles) OR (user_staff_roles.id = auth.uid())))))
with check ((auth.uid() IN ( SELECT user_staff_roles.id AS user_id
   FROM user_staff_roles
  WHERE ((user_staff_roles.staff_role = 'admin'::staff_roles) OR (user_staff_roles.id = auth.uid())))));
